...
import com.falldetection.ui.ContactsScreen
    ...
    private val fallDetectionReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == SensorMonitoringService.ACTION_FALL_DETECTED) {
                val event = intent.getParcelableExtra<com.falldetection.model.FallDetectionEvent>("event")
                val confidence = intent.getFloatExtra("confidence", 0f)
                if (event != null) {
                    alertViewModel.setFallAlert(event, confidence)
...
fun MainApp(
    ...
    val alertState = alertViewModel.alertState
